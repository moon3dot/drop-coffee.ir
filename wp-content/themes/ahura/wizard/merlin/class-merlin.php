...
class Merlin {
	...
	function __construct( $config = array(), $strings = array() ) {
		...
		function ahura_redirect() {
		    if( isset( $_GET['activated'] ) ) {
		        wp_safe_redirect( admin_url('admin.php?page=ahura-wizard') );
		        exit;
		    }
		}
		...
		add_action( 'wp_ajax_merlin_generate_demo_file', array( $this, '_ajax_generate_demo_file' ), 10, 0 );
		add_action( 'wp_ajax_merlin_content', array( $this, '_ajax_content' ), 10, 0 );
		add_action( 'wp_ajax_merlin_content_media', array( $this, '_ajax_content_media' ), 10, 0 );
	...
	function required_classes() {
		...
		$fetch_attachments = get_option('ahura_merlin_fetch_attachments_status');
		$fetch_attachments = (bool) ($fetch_attachments === 'true' || $fetch_attachments === true);
		$fetch_attachments = $fetch_attachments ? false : true;

		require_once trailingslashit( $this->base_path ) . $this->directory . '/includes/class-merlin-downloader.php';

		$this->importer = new ProteusThemes\WPContentImporter2\Importer( array( 'fetch_attachments' => $fetch_attachments ), $this->logger );
	...
	public function admin_page() {
        $license_status = \ahura\app\license::is_active();
        ...
        if (!wp_doing_ajax() && $this->step && !in_array($this->step, ['license', 'welcome']) && !$license_status){
            wp_redirect(admin_url('admin.php?page=ahura-wizard&step=license'));
            return false;
        }
		...
		// Enqueue javascript.
        wp_enqueue_script('ahura_assets_js', get_template_directory_uri(). '/js/assets.js' , ['jquery'], MERLIN_VERSION , true);
        ...
        if (Studio::is_studio()){
            wp_enqueue_style('mw_ahura_elementor_font_css', get_template_directory_uri() . '/css/customization/dana.css', [], MERLIN_VERSION);
            \ahura\app\mw_assets::load_admin_assets(null);
        }

        $texts = array(
			'something_went_wrong' => esc_html__( 'Something went wrong. Please refresh the page and try again!', 'ahura' ),
			'import_sideload_label' => esc_html__( 'Enable this option to install faster and avoid consuming hosting resources.', 'ahura' ),
			'import_sideload_tooltip' => esc_html__( 'If you are importing on localhost or if you encounter any problem in importing the demo, if you are using vpn, please turn it off and import again.', 'ahura' ),
            'plz_wait' => esc_html__( 'Please Wait...', 'ahura' ),
            'storage_check' => sprintf(__('The host has at least %s %s of space.', 'ahura'), '300', __('MB', 'ahura')),
            'required_input' => __('Check this option is required.', 'ahura'),
        );
	...
	protected function header() {
		...
		$current_step_slug = isset($_GET['step']) && !empty($_GET['step']) ? $_GET['step'] : 'main';
		...
		<body class="merlin__body merlin__body-step--<?php echo esc_attr( $current_step_slug ); ?> merlin__body--<?php echo esc_attr( $current_step ); ?><?php echo Studio::is_studio() ? ' is-studio-page' : '' ?>">
		<?php
	}
	...
	protected function plugins() {
		...
		<div class="merlin__content--transition">
            ...
            <hr>
            <p class="req-msg"><?php _e('The required plugins in the list below must be installed and activated.', 'ahura') ?></p>
            <?php endif; ?>
			...
			<?php if ( $count ) : ?>
				...
				<?php if ( ! empty( $required_plugins ) ) : ?>
					<?php foreach ( $required_plugins as $slug => $plugin ) : ?>
						<li class="required-feature" data-slug="<?php echo esc_attr( $slug ); ?>">
				...
				<?php if ( ! empty( $recommended_plugins ) ) : ?>
					<?php foreach ( $recommended_plugins as $slug => $plugin ) : ?>
						<li class="recommended-feature" data-slug="<?php echo esc_attr( $slug ); ?>">
							<input type="checkbox" name="default_plugins[<?php echo esc_attr( $slug ); ?>]" class="checkbox" id="default_plugins_<?php echo esc_attr( $slug ); ?>" value="1" checked>
	...
	protected function content() {
        ...
        $has_minimum_php = \ahura\app\mw_config::has_minimum_php_version();
        ...
        include_once get_template_directory() . '/template-parts/admin/pages/studio.php';
    ...
    function _ajax_generate_demo_file(){
        check_ajax_referer('merlin_nonce', 'wpnonce');

        $importer = new \ahura\app\Studio_Importer();
        $importer->set_demo_id($_POST['selected_index']);
        $importer->generate_demo_file();

        if(file_exists($importer->get_demo_file_path())){
	...
	function _ajax_content() {
        ...
        $result = $importer->import_demo([
                'demo' => $_POST['selected_index'],
                'step' => $_POST['step'],
        ...
        if($result){
            wp_send_json([
                'message' => __('Demo import was done successfully.', 'ahura'),
                'type' => 'success',
                'data' => $result,
                'site_url' => site_url()
            ]);
            ...
            wp_send_json([
                'message' => __('Demo import failed.', 'ahura'),
                'type' => 'error'
            ]);
        }
	}

    function _ajax_content_media() {
        check_ajax_referer('merlin_nonce', 'wpnonce');
	...
	public function get_import_data_info( $selected_import_index = 0 ) {
        ...
        if (
            ! empty( $this->import_files[ $selected_import_index ]['import_url'] ) ||
	...
	public function get_import_files_paths( $selected_import_index ) {
		...
		if ( empty( $selected_import_data['import_url'] ) ) {
			...
			if ( empty( $import_files['content'] ) ) {
				$import_files['content'] = $downloader->download_file( $selected_import_data['import_url'], $content_filename );
			}
