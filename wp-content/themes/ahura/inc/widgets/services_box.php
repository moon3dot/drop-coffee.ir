...
class services_box extends \Elementor\Widget_Base {
	...
	{
		return ['servicebox', 'services_box', 'services_box', esc_html__( 'Services Box' , 'ahura')];
	}
	function __construct($data=[], $args=null)
	{
		parent::__construct($data, $args);
		$services_box_css = mw_assets::get_css('elementor.services_box');
		mw_assets::register_style('services_box', $services_box_css);
	}
	function get_style_depends()
	{
		return [mw_assets::get_handle_name('services_box')];
	}
		...
		$this->add_control(
			...
			[
				...
				'selectors' => [
					...
					'{{WRAPPER}} .icon_section svg' => 'fill: {{VALUE}}'
				]
			]
		);
		$this->add_control(
	...
	protected function render_service_icon($settings)
	{
		...
		{
			echo '<img width="70" height="70" src="'.$settings['service_icon']['value']['url'].'" />';
		}else{
		...
		?>
			<div class="icon_wrapper">
				<div class="icon_section" <?php echo $settings['ah_no_icon_padding'] ? 'style="padding: 0;"' : ''?>>
					<?php $this->render_service_icon($settings); ?>
				</div>
			</div>
			<div class="ah_title"><?php $this->render_inline_edit_data($title, 'service_title'); ?></div>
			<p class="description"><?php $this->render_inline_edit_data($content, 'service_description');?></p>
		<?php
